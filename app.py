...
# ===== CCD NATURAL CHAT START =====
def _ccd_is_natural_chat(message):
    q = clean_user_message(message).lower().strip()

    keywords = [
        # ทักทาย / คุยเล่น
        "สวัสดี", "หวัดดี", "ฮัลโหล", "ดีจ้า", "ดีค่ะ", "ดีครับ",
        "ทำไร", "อยู่ไหม", "อยู่มั้ย", "ตอบหน่อย", "ช่วยหน่อย",

        # ถามเมนู / แนะนำ
        "เมนู", "ขอดูเมนู", "ดูเมนู", "มีเมนูอะไรบ้าง", "มีอะไรขาย",
        "ขายอะไร", "แนะนำ", "แนะนำหน่อย", "เมนูแนะนำ", "เมนูขายดี",
        "ขายดี", "ยอดนิยม", "ตัวไหนดี", "อันไหนดี", "กินไรดี", "สั่งไรดี",
        "คุกกี้ไหนดี", "อะไรอร่อย", "อร่อยสุด", "น่ากิน", "อยากกิน",
        "เลือกไม่ถูก", "ลังเล", "จัดให้หน่อย",

        # ราคา / คุ้มไหม
        "ราคา", "กี่บาท", "เท่าไหร่", "แพงไหม", "แพงมั้ย", "แพงจัง",
        "คุ้มไหม", "คุ้มมั้ย", "มีงบ", "งบ", "ประหยัด", "ถูกสุด",

        # โปรโมชัน
        "โปร", "โปรโมชั่น", "โปรโมชัน", "รับโปร", "มีโปรไหม",
        "ส่วนลด", "ลดไหม", "ลดมั้ย", "แถม", "cookie fortune",
        "ไพ่", "สุ่มไพ่", "ครบ 150",

        # การสั่งซื้อ
        "สั่งยังไง", "สั่งซื้อ", "อยากสั่ง", "ซื้อยังไง",
        "เอา", "รับ", "ขอ", "เพิ่มออเดอร์", "สั่งได้ไหม",

        # ชม / รีวิว
        "น่ารัก", "บริการดี", "ดีจัง", "ชอบ", "ชอบมาก",
        "ประทับใจ", "ขอบคุณ", "ขอบใจ", "ร้านดี",

        # รสชาติ
        "หวานน้อย", "หวานมาก", "ช็อกโกแลต", "ช็อกโกแลตเข้ม",
        "คาราเมล", "มัทฉะ", "เนยสด", "กรอบ", "นุ่ม", "หอม",
        "ผลไม้", "เปรี้ยว", "เด็กกินได้ไหม",
    ]

    return any(word in q for word in keywords)


def _ccd_get_natural_chat_reply(message):
    q = clean_user_message(message).lower().strip()

    # ทักทาย
    if any(word in q for word in ["สวัสดี", "หวัดดี", "ฮัลโหล", "ดีจ้า", "ดีค่ะ", "ดีครับ"]):
        return "สวัสดีค่า ☁️🍪 Demi อยู่ตรงนี้แล้วน้า อยากดูเมนู โปร หรือให้ช่วยเลือกคุกกี้ บอกได้เลยค่ะ 🤎"

    # เมนู / แนะนำ / ขายดี
    if any(word in q for word in [
        "เมนู", "ขอดูเมนู", "ดูเมนู", "มีเมนูอะไรบ้าง", "มีอะไรขาย",
        "ขายอะไร", "แนะนำ", "เมนูแนะนำ", "ขายดี", "ยอดนิยม",
        "กินไรดี", "สั่งไรดี", "คุกกี้ไหนดี", "อะไรอร่อย", "เลือกไม่ถูก"
    ]):
        return _ccd_get_bestseller_reply_v2()

    # โปร
    if any(word in q for word in ["โปร", "โปรโมชั่น", "โปรโมชัน", "รับโปร", "ส่วนลด", "แถม", "ไพ่", "สุ่มไพ่", "ครบ 150"]):
        return get_promotion_reply()

    # ราคา / แพง / คุ้ม
    if any(word in q for word in ["แพง", "คุ้ม", "ราคา", "กี่บาท", "เท่าไหร่", "มีงบ", "งบ", "ประหยัด", "ถูกสุด"]):
        return (
            "ไม่แพงมากค่า ถ้าเทียบกับคุกกี้โฮมเมดทำสด ๆ และใช้วัตถุดิบดี ๆ ☁️🍪\n\n"
            "ถ้าบอกงบมาได้เลย เช่น “มีงบ 100 บาท” Demi จะช่วยจัดเมนูที่คุ้ม ๆ ให้ค่ะ 🤎"
        )

    # รสชาติ
    if any(word in q for word in ["หวานน้อย", "หวานมาก", "ช็อกโกแลต", "คาราเมล", "มัทฉะ", "เนยสด", "กรอบ", "นุ่ม", "หอม"]):
        return (
            "ได้เลยค่า ☁️🍪 ถ้าลูกค้าบอกแนวที่ชอบมาแบบนี้ Demi ช่วยเลือกให้ได้เลยค่ะ\n\n"
            "ถ้าชอบช็อกโกแลต จะไปทางเข้ม ๆ หอม ๆ\n"
            "ถ้าชอบคาราเมล จะหวานหอมละมุน\n"
            "ถ้าชอบหวานน้อย Demi จะแนะนำตัวที่กินง่าย ไม่เลี่ยนให้ค่ะ 🤎"
        )

    # ชม / รีวิว
    if any(word in q for word in ["บริการดี", "ดีจัง", "น่ารัก", "ชอบ", "ประทับใจ", "ร้านดี"]):
        st.session_state.show_review_form = True
        return "ขอบคุณมากเลยค่า ดีใจที่ชอบร้านนะคะ ☁️🍪 ถ้าสะดวก Demi ขอชวนให้คะแนนร้านนิดนึงนะคะ 🤎"

    # ขอบคุณ
    if any(word in q for word in ["ขอบคุณ", "ขอบใจ"]):
        return "ยินดีมากค่า ☁️🍪 ถ้ามีอะไรให้ Demi ช่วยอีก พิมพ์มาได้เลยนะคะ 🤎"

    # สั่งซื้อ
    if any(word in q for word in ["สั่งยังไง", "สั่งซื้อ", "อยากสั่ง", "ซื้อยังไง"]):
        return (
            "สั่งผ่าน Demi ได้เลยค่ะ 🍪\n\n"
            "ลูกค้าพิมพ์ชื่อเมนูพร้อมจำนวนได้เลย เช่น\n"
            "“เอาคุกกี้ช็อกโกแลตชิพ 2 ชิ้น”\n\n"
            "หรือถ้ายังเลือกไม่ถูก พิมพ์ว่า “เมนูแนะนำ” ได้เลยค่ะ"
        )

    return "ได้เลยค่า ☁️🍪 Demi ช่วยเรื่องเมนู ราคา โปร หรือช่วยเลือกคุกกี้ให้ได้นะคะ"
# ===== CCD NATURAL CHAT END =====
...
# ===== CCD SOFT ERROR HANDLER START =====
def _ccd_customer_safe_error_reply():
    return (
        "ขออภัยค่า Demi งงนิดนึงกับคำถามนี้ ☁️🍪\n\n"
        "ลองพิมพ์ใหม่อีกครั้งได้ไหมคะ เช่น “เมนูแนะนำ”, “รับโปร”, "
        "“สั่งคุกกี้ช็อกโกแลตชิพ 2 ชิ้น” หรือบอกงบให้ Demi ช่วยเลือกเมนูได้เลยค่ะ 🤎"
    )


def _ccd_soft_error(message="", *args, **kwargs):
    # ไม่โชว์ error สีแดงให้ลูกค้าเห็น
    return st.info(_ccd_customer_safe_error_reply())


st.error = _ccd_soft_error
# ===== CCD SOFT ERROR HANDLER END =====
...
PROMO_REPLY = """ตอนนี้ CookieCloudyDay มีโปรโมชันของร้านตามนี้ค่ะ ☁️🍪
...
def render_menu_order_popup():
    ...
    if not menus:
        st.info("ยังโหลดเมนูไม่ได้ค่ะ กรุณาตรวจสอบ shop_menu.json")
        return
...
def _ccd_save_review_to_sheet(rating, review_text):
    worksheet = _ccd_get_review_worksheet()
    ...
    with st.form("cookiecloudyday_review_form"):
        ...
        if submitted_review:
            try:
                ...
                st.info(f"บันทึกรีวิวลงชีทไม่สำเร็จค่ะ: {e}")
# ===== CCD REVIEW FORM END =====
...
if prompt:
    # CCD_SAFE_PROMPT_TRY_START
    try:
        # CCD_NATURAL_CHAT_DIRECT_START
        prompt = clean_user_message(prompt)
    
        if _ccd_is_natural_chat(prompt):
            answer = _ccd_get_natural_chat_reply(prompt)
        ...
        user_text = clean_user_message(prompt)
        if user_text:
            already_last_user = (
                len(st.session_state.messages) > 0
                and st.session_state.messages[-1].get("role") == "user"
                and st.session_state.messages[-1].get("content") == user_text
            )
            if not already_last_user:
                st.session_state.messages.append({"role": "user", "content": user_text})

        st.session_state.messages.append({
            "role": "assistant",
            "content": _ccd_customer_safe_error_reply(),
        })
        st.rerun()
    # CCD_SAFE_PROMPT_TRY_END
