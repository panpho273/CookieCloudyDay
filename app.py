...
def direct_intent_reply(prompt: str):
    q = (prompt or "").strip().lower()

    if q in ["ซื้อ", "สั่ง", "สั่งซื้อ", "order", "ออเดอร์"]:
        return (
            "ได้เลยค่ะ รับคุกกี้อะไรดีคะ 🍪\n\n"
            "ลูกค้าพิมพ์ชื่อเมนูพร้อมจำนวนได้เลย เช่น\n"
            "“รับคุกกี้ช็อกโกแลตชิพ 2 ชิ้น”\n"
            "หรือ “เอาคุกกี้เลมอนบัตเตอร์ 3 ชิ้น”\n\n"
            "โปรเปิดร้าน: ซื้อครบ 150 บาทขึ้นไป ได้สุ่ม Lucky Cookie Tarot "
            "และรับฟรีคุกกี้ตามไพ่ที่สุ่มได้ 1 ชิ้นค่ะ 🔮"
        )

    if any(word in q for word in ["โปร", "โปรโมชั่น", "แถม", "ไพ่", "ทาโร่", "tarot"]):
        return (
            "โปรเปิดร้านตอนนี้คือ 🎁\n\n"
            "ซื้อครบ 150 บาทขึ้นไป\n"
            "→ ได้สุ่ม Lucky Cookie Tarot\n"
            "→ ได้คำทำนายประจำวัน\n"
            "→ รับฟรีคุกกี้ตามไพ่ที่สุ่มได้ 1 ชิ้นค่ะ"
        )

    if any(word in q for word in ["เปิดกี่โมง", "ร้านเปิด", "เวลาเปิด", "ปิดกี่โมง"]):
...
def fallback_answer(user_question: str) -> str:
    ...
    return "ได้เลยค่ะ รับคุกกี้อะไรดีคะ 🍪 ลูกค้าพิมพ์ชื่อเมนูพร้อมจำนวนได้เลยค่ะ"
...
if prompt:
    ...
    direct_answer = direct_intent_reply(normalized_prompt)
    if direct_answer:
